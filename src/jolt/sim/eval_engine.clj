(ns jolt.sim.eval-engine
  ...
  A persistent adapter owns `:ns` as its evaluator coordinate: pass the prior
  result's `:ns` into the next request and do not dynamically bind `*ns*`
  around `evaluate`, because that binding masks Jolt's runtime namespace
  transition."
