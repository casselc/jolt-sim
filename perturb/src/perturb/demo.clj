...
(defn claim-3 []
  ...
  (let [ci (cap/checker-input)]
    ...
    (println "    - every accepted program is also EXECUTED under the scripted")
    (println "      handler. E15 found the first accept set type-checked and threw.")
    (println "    - perturb.nrepl itself now checks: clone-session, eval-code and")
    (println "      session carry positioned signatures (:arg n / :at [i]) that say")
    (println "      WHICH position of [conn value] holds the capability.")
    (println "    - but 8 of its 15 functions are AXIOMS whose bodies are believed,")
    (println "      not checked. Run -M:check and read the limits block before")
    (println "      believing any of this. PERTURB-DESIGN E15, E17.")
    (println)
