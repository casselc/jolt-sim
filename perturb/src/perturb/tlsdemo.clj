...
(defn -main [& args]
  (let [port (if (seq args) (Integer/parseInt (first args)) 7931)]
    ...
    (println)
    (println "  CONSEQUENCE. Layering is possible but it is not provided: the layer")
    (println "  must rebind the effect name to the rung below for exactly the extent")
    (println "  of its inner perform. perturb.tlsish/via is that rebinding, it is one")
    (println "  line, and NOTHING CHECKS IT. A layer that forgot would loop rather")
    (println "  than be refused — the residual shape perturb.posix/defsys closed one")
    (println "  rung down, reopened one rung up.")
                ...
                (let [sp (chk/spec)
                  ...
                  (let [run-c (fx/new-run #{'perturb.wire/socket.listen})
                    ...
                    (let [hs    ((f 'perturb.tlsish/frame)
                      ...
                      (println "                      try/finally.")
                      (println "  failed              the effect boundary as a SEAM. Control 3")
                      (println "                      runs the same layer composed by CALLING,")
                      (println "                      gets identical octets, and no instrument in")
                      (println "                      perturb distinguishes the two runs. Using")
                      (println "                      the boundary is voluntary at every rung")
                      (println "                      above the native one.")
                      (println)
