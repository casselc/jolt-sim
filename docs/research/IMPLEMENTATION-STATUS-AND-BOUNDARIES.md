# jolt-sim implementation status and boundaries

This is the detailed technical record that formerly occupied the repository
landing page. It records current contracts, executed lanes, and limits. Start
with the short [repository README](../../README.md) when you want to use the
project.

The claims here are scoped to the named code and gates. They are not a release
compatibility promise or a claim that arbitrary Jolt programs are model
checked.
...
[`docs/ROADMAP.md`](../ROADMAP.md). The ordinary HTTP -> SQLite -> TCP/bencode
